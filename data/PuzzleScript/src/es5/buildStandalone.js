var get_blob = function get_blob() {
  return self.Blob;
};
...
function buildStandalone(sourceCode) {
  ...
  htmlString = htmlString.replace(/__HOMEPAGE_STRIPPED_PROTOCOL__/g, homepage_stripped); // $ has special meaning to JavaScript's String.replace 
  ...
  sourceCode = sourceCode.replace(/\$/g, '$$$$'); // Then when we substitute them, the doubled $'s will be reduced to single ones.
