...
pickupCoin = function pickupCoin() {
  var result = Params();
...
laserShoot = function laserShoot() {
  var result = Params();
...
birdSound = function birdSound() {
  ...
  result.p_freq_ramp = 0.3 + frnd(0.15); //  result.p_freq_dramp = 0.3+frnd(2.0);
...
blipSelect = function blipSelect() {
  result = Params();
...
generateFromSeed = function generateFromSeed(seed) {
...
SoundEffect.generate = function (ps) {
  ...
  var flthp_d = 1.0 + ps.p_hpf_ramp * 0.0003; // Vibrato

  var vib_phase = 0.0;
  var vib_speed = Math.pow(ps.p_vib_speed, 2.0) * 0.01;
  var vib_amp = ps.p_vib_strength * 0.5; // Envelope
  ...
  var env_total_length = env_length[0] + env_length[1] + env_length[2]; // Phaser
  ...
  if (ps.p_repeat_speed == 0.0) rep_limit = 0; //var gain = 2.0 * Math.log(1 + (Math.E - 1) * ps.sound_vol);

  var gain = 2.0 * ps.sound_vol;
  var gain = Math.exp(ps.sound_vol) - 1;
  var num_clipped = 0; // ...end of initialization. Generate samples.
  ...
  for (var t = 0;; ++t) {
    ...
    if (square_duty > 0.5) square_duty = 0.5; // Volume envelope
    ...
    for (var si = 0; si < 8; ++si) {
      ...
      if (phase >= period) {
        phase %= period;
        if (ps.wave_type === NOISE) for (var i = 0; i < 32; ++i) {
          noise_buffer[i] = Math.random() * 2.0 - 1.0;
        }
      } // Base waveform
