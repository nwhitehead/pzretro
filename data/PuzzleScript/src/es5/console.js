function jumpToLine(i) {
  ...
  var editor = code.editorreference; // editor.getLineHandle does not help as it does not return the reference of line.
...
function consolePrintFromRule(text, rule, urgent) {
  ...
  var logString = '<font color="green">Rule <a onclick="jumpToLine(' + rule.lineNumber + ');"  href="javascript:void(0);">' + rule.lineNumber + '</a> ' + ruleDirection + " : " + text + '</font>';
...
function consoleCacheDump() {
  ...
  for (var i = 0; i < consolecache.length - 1; i++) {
    ...
    if (this_row_text === next_row_text) {
      ...
      i--; //need to preserve visual_id from later one
  ...
  var current_batch_row = []; //pass 2 : group by debug visibility
  ...
  for (var j = 0; j < batched_messages.length; j++) {
    ...
    if (inspector_ID != null) {
      summarised_message += "<span class=\"hoverpreview\" onmouseover=\"debugPreview(".concat(inspector_ID, ")\" onmouseleave=\"debugUnpreview()\">");
    }
    ...
    for (var i = 0; i < batch_messages.length; i++) {
      ...
      if (times_repeated > 1) {
        curline += " (x".concat(times_repeated, ")");
      }
...
function clearConsole() {
  ...
  code.innerHTML = '';
  var objDiv = document.getElementById('lowerarea');
  objDiv.scrollTop = objDiv.scrollHeight; //clear up debug stuff.
