...
var intro_template = ["..................................", "..................................", "..................................", "......Puzzle Script Terminal......", "..............v 1.7...............", "..................................", "..................................", "..................................", ".........insert cartridge.........", "..................................", "..................................", "..................................", ".................................."];
  ...
  if (leveldat.message === undefined) {
    ...
    if (state !== undefined) {
      if (state.metadata.flickscreen !== undefined) {
        ...
        oldflickscreendat = [0, 0, Math.min(state.metadata.zoomscreen[0], level.width), Math.min(state.metadata.zoomscreen[1], level.height)];
      }
    }
...
function RebuildLevelArrays() {
  ...
  level.mapCellContents_Movements = new BitVec(STRIDE_MOV); //I have these to avoid dynamic allocation - I generate 3 because why not, 
...
function restoreLevel(lev) {
  ...
  if (level.width !== lev.width || level.height !== lev.height) {
    ...
    RebuildLevelArrays(); //regenerate all other stride-related stuff
  } else {
...
var screenwidth = 0;
var screenheight = 0; //compresses 'before' into diff
...
function repositionEntitiesOnLayer(positionIndex, layer, dirMask) {
  ...
  level.rowCellContents[rowIndex].ior(movingEntities); //corresponding movement stuff in setmovements
...
Level.prototype.delta_index = function (direction) {
  var _dirMasksDelta$direct = _slicedToArray(dirMasksDelta[direction], 2),
      dx = _dirMasksDelta$direct[0],
      dy = _dirMasksDelta$direct[1];
...
Level.prototype.setMovements = function (index, vec) {
  ...
  var targetIndex = index * STRIDE_MOV + i; //corresponding object stuff in repositionEntitiesOnLayer
...
CellPattern.prototype.replace = function (rule, currentIndex) {
  ...
  var result = false; //check if it's changed
...
function matchCellRow(direction, cellRowMatch, cellRow, cellRowMask, cellRowMask_Movements, d) {
  ...
  if (!cellRowMask.bitsSetInArray(level.mapCellContents.data) || !cellRowMask_Movements.bitsSetInArray(level.mapCellContents_Movements.data)) {
    return result;
  }
  ...
  if (horizontal) {
    for (var y = ymin; y < ymax; y++) {
      if (!cellRowMask.bitsSetInArray(level.rowCellContents[y].data) || !cellRowMask_Movements.bitsSetInArray(level.rowCellContents_Movements[y].data)) {
        continue;
      }
    ...
    for (var x = xmin; x < xmax; x++) {
      if (!cellRowMask.bitsSetInArray(level.colCellContents[x].data) || !cellRowMask_Movements.bitsSetInArray(level.colCellContents_Movements[x].data)) {
        continue;
      }
...
Rule.prototype.findMatches = function () {
  ...
  var d = level.delta_index(this.direction);
  var matches = [];
...
Rule.prototype.applyAt = function (level, tuple, check, delta) {
  var rule = this; //have to double check they apply 
  //(cf test ellipsis bug: rule matches two candidates, first replacement invalidates second)

  if (check) {
    for (var cellRowIndex = 0; cellRowIndex < this.patterns.length; cellRowIndex++) {
      if (this.ellipsisCount[cellRowIndex] === 1) {
        if (this.cellRowMatches[cellRowIndex](this.patterns[cellRowIndex], tuple[cellRowIndex][0], tuple[cellRowIndex][1] + 1, tuple[cellRowIndex][1], delta, level.objects, level.movements).length == 0) return false;
      } else if (this.ellipsisCount[cellRowIndex] === 2) {
        if (this.cellRowMatches[cellRowIndex](this.patterns[cellRowIndex], tuple[cellRowIndex][0], tuple[cellRowIndex][1] + tuple[cellRowIndex][2] + 1, tuple[cellRowIndex][1] + tuple[cellRowIndex][2], tuple[cellRowIndex][1] + 1, tuple[cellRowIndex][1], tuple[cellRowIndex][2] + 1, tuple[cellRowIndex][2], delta, level.objects, level.movements).length == 0) return false;
      } else {
        if (!this.cellRowMatches[cellRowIndex](this.patterns[cellRowIndex], tuple[cellRowIndex], delta, level.objects, level.movements)) return false;
      }
    }
  }

  var result = false;
  var anyellipses = false; //APPLY THE RULE
  ...
  if (verbose_logging && result) {
    ...
    var logString = "<font color=\"green\">Rule <a onclick=\"jumpToLine(".concat(rule.lineNumber, ");\"  href=\"javascript:void(0);\">").concat(rule.lineNumber, "</a> ").concat(ruleDirection, " applied").concat(gapMessage, ".</font>");
...
Rule.prototype.tryApply = function (level) {
  var delta = level.delta_index(this.direction); //get all cellrow matches
...
function applyRandomRuleGroup(level, ruleGroup) {
  ...
  var tuple = match[1];
  var check = false;
  var delta = level.delta_index(rule.direction);
...
function applyRuleGroup(ruleGroup) {
  ...
  while (propagated) {
    ...
    for (var ruleIndex = 0; ruleIndex < ruleGroup.length; ruleIndex++) {
      ...
      if (nothing_happened_counter === ruleGroup.length) break;
    }
...
function applyRules(rules, loopPoint, startRuleGroupindex, bannedGroup) {
  ...
  for (var ruleGroupIndex = startRuleGroupindex; ruleGroupIndex < rules.length;) {
    if (bannedGroup && bannedGroup[ruleGroupIndex]) {//do nothing
    } else {
...
function resolveMovements(level, bannedGroup) {
  ...
  var doUndo = false; //Search for any rigidly-caused movements remaining
          ...
          for (var j = 0; j < level.layerCount; j++) {
            ...
            if (layerSection !== 0) {
              ...
              if (bannedGroup[groupIndex] !== true) {
                bannedGroup[groupIndex] = true; //backtrackTarget = rigidBackups[rigidGroupIndex];
  ...
  if (dir <= 4) {
    ...
    if (verbose_logging) {
      ...
      if (dir === -1) {
        ...
        //  consolePrint('=======================');
        consolePrint("Turn starts with input of ".concat(['up', 'left', 'down', 'right', 'action'][inputindex], "."), false, null, inspect_ID);
      }
    }
      ...
      if (shouldUndo) {
        rigidloop = true;
        {
          ...
          if (IDE) {
            ...
            var bannedLineNumbers = newBannedGroups.map(function (rgi) {
              return state.rules[rgi][0].lineNumber;
            });
            var ts = bannedLineNumbers.length > 1 ? "lines " : "line ";
            ts += bannedLineNumbers.map(function (ln) {
              return "<a onclick=\"jumpToLine(".concat(ln, ");\" href=\"javascript:void(0);\">").concat(ln, "</a>");
            }).join(", ");
            consolePrint("Rigid movement application failed in rule-Group starting from ".concat(ts, ", and will be disabled in resimulation. Rolling back..."));
          ...
          level.rigidMovementAppliedMask = startState.rigidMovementAppliedMask.concat([]); // TODO: shouldn't we also save/restore the level data computed by level.calculateRowColMasks() ?

          level.commandQueue = startState.commandQueue.concat([]);
          level.commandQueueSourceRules = startState.commandQueueSourceRules.concat([]);
          sfxCreateMask.setZero();
          sfxDestroyMask.setZero(); // TODO: should
        }
    ...
    if (!winning) {
      ...
      if (level.commandQueue.indexOf('again') >= 0 && modified) {
        var r = level.commandQueueSourceRules[level.commandQueue.indexOf('again')]; //first have to verify that something's changed
...
function checkWin(dontDoWin) {
  ...
  if (state.winconditions.length > 0) {
    ...
    for (var wcIndex = 0; wcIndex < state.winconditions.length; wcIndex++) {
      ...
      var rulePassed = true;
      var f1 = aggr1 ? function (c) {
        return filter1.bitsSetInArray(c);
      } : function (c) {
        return !filter1.bitsClearInArray(c);
      };
      var f2 = aggr2 ? function (c) {
        return filter2.bitsSetInArray(c);
      } : function (c) {
        return !filter2.bitsClearInArray(c);
      };
...
function goToTitleScreen() {
  ...
  if (canvas !== null) {
    //otherwise triggers error in cat bastard test
    regenSpriteImages();
  }
}