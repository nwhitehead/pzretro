...
(function (mod) {
  ...
  var RULE_COMMAND_WORDS = ["COMMAND", "sfx0", "sfx1", "sfx2", "sfx3", "sfx4", "sfx5", "sfx6", "sfx7", "sfx8", "sfx9", "sfx10", "cancel", "checkpoint", "restart", "win", "message", "again"];
  var CARDINAL_DIRECTION_WORDS = ["DIRECTION", "up", "down", "left", "right", "horizontal", "vertical"];
  var RULE_DIRECTION_WORDS = ["DIRECTION", //tag
  "up", "down", "left", "right", "random", "horizontal", "vertical", "late", "rigid"];
  var LOOP_WORDS = ["BRACKET", //tag
  "startloop", "endloop"];
  ...
  var SOUND_EVENTS = ["SOUNDEVENT", "undo", "restart", "titlescreen", "startgame", "cancel", "endgame", "startlevel", "endlevel", "showmessage", "closemessage", "sfx0", "sfx1", "sfx2", "sfx3", "sfx4", "sfx5", "sfx6", "sfx7", "sfx8", "sfx9", "sfx10"];
  var SOUND_VERBS = ["SOUNDVERB", "move", "action", "create", "destroy", "cantmove"];
  ...
  var PRELUDE_COLOR_PALETTE_WORDS = ["mastersystem", "gameboycolour", "amiga", "arnecolors", "famicom", "atari", "pastel", "ega", "amstrad", "proteus_mellow", "proteus_rich", "proteus_night", "c64", "whitingjp"];
  ...
  CodeMirror.registerHelper("hint", "anyword", function (editor, options) {
    ...
    var state = tok.state; // ignore empty word
    ...
    switch (state.section) {
      ...
      case 'legend':
        {
          var splits = lineToCursor.toLowerCase().split(/[\t-\r \xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\uFEFF]/).filter(function (v) {
            return v !== '';
          });
          toexclude = splits.filter(function (a) {
            return LEGEND_LOGICWORDS.indexOf(a) === -1;
          }); //don't filter out and or or
      ...
      default:
        //preamble
        {
          ...
          if (lc.indexOf("background_color") >= 0 || lc.indexOf("text_color") >= 0) {
            candlists.push(COLOR_WORDS);
          } else {
    ...
    var list = options && options.list || [],
        seen = {}; //first, add objects if needed
    ...
    if (addObjects) {
      ...
      for (var key in obs) {
        if (obs.hasOwnProperty(key)) {
          var w = key;
          var matchWord = w.toLowerCase(); // if (matchWord === curWord) continue;
    ...
    for (var i = 0; i < candlists.length; i++) {
      ...
      for (var j = 1; j < candlist.length; j++) {
        ...
        var matchWord = m;
        var matchWord = matchWord.toLowerCase(); // if (matchWord === curWord) continue;
