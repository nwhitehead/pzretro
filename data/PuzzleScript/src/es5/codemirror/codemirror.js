...
(function (mod) {
  ...
  var captureRightClick = gecko || ie && !ie_upto8; // Optimize some code when these features are not used.

  var sawReadOnlySpans = false,
      sawCollapsedSpans = false; // EDITOR CONSTRUCTOR
  ...
  function CodeMirror(place, options) {
    ...
    runInOp(this, function () {
      ...
      for (var opt in optionHandlers) {
        if (optionHandlers.hasOwnProperty(opt)) optionHandlers[opt](cm, options[opt], Init);
      }
  ...
  function Display(place, doc) {
    var d = this; // The semihidden textarea that is focused when the editor is
    ...
    d.scrollbarV = elt("div", [elt("div", null, null, "min-width: 1px")], "CodeMirror-vscrollbar"); // Covers bottom-right square when both scrollbars are present.
    ...
    if (place.appendChild) place.appendChild(d.wrapper);else place(d.wrapper); // Current rendered range (may be bigger than the view window).
    ...
    d.viewOffset = 0;
    d.lastSizeC = 0;
    d.updateLineNumbers = null; // Used to only resize the line number gutter when necessary (when
    ...
    d.poll = new Delayed();
    d.cachedCharWidth = d.cachedTextHeight = d.cachedPaddingH = null; // Tracks when resetInput has punted to just putting a short
  ...
  function updateScrollbars(cm, measure) {
    ...
    if (needsV) {
      ...
      d.scrollbarV.style.bottom = needsH ? scrollbarWidth(d.measure) + "px" : "0"; // A bug in IE8 can cause this value to be negative, so guard it.
  ...
  function updateDisplay(cm, viewPort, forced) {
    ...
    for (var first = true;; first = false) {
      ...
      updated = true; // If the max line changed since it was last measured, measure it,
      ...
      forced = false; // Clip forced viewport to actual scrollable area.
  ...
  function updateDisplayInner(cm, visible, forced) {
    ...
    var dims = getDimensions(cm); // Compute a suitable new viewport (from & to)
    ...
    if (!different && toUpdate == 0 && !forced) return; // For big changes, we hide the enclosing element during the
    // update, since that speeds up the operations on most browsers.

    var focused = activeElt();
    if (toUpdate > 4) display.lineDiv.style.display = "none";
    patchDisplay(cm, display.updateLineNumbers, dims);
    if (toUpdate > 4) display.lineDiv.style.display = ""; // There might have been a widget with a focused element that got
  ...
  function updateSelectionCursor(cm, range, output) {
    var pos = _cursorCoords(cm, range.head, "div");
  ...
  function mapFromLineView(lineView, line, lineN) {
    ...
    for (var i = 0; i < lineView.rest.length; i++) {
      if (lineNo(lineView.rest[i]) > lineN) return {
        map: lineView.measure.maps[i],
        cache: lineView.measure.caches[i],
        before: true
      };
  ...
  function measureCharInner(cm, prepared, ch, bias) {
    ...
    var node, start, end, collapse; // First, search the line map for the text node corresponding to,
  ...
  function _cursorCoords(cm, pos, context, lineObj, preparedMeasure) {
  ...
  function startOperation(cm) {
    cm.curOp = {
      ...
      // Whether to fire a cursorActivity event
  ...
  function endOperation(cm) {
    var op = cm.curOp,
        ...
        display = cm.display;
    cm.curOp = null;
    if (op.updateMaxLine) findMaxLine(cm); // If it looks like an update might be needed, call updateDisplay
    ...
    if (!updated && op.startHeight != cm.doc.height) updateScrollbars(cm); // Propagate the scroll position to the actual DOM scroller
    ...
    if (op.scrollToPos) {
      var coords = scrollPosIntoView(cm, _clipPos(cm.doc, op.scrollToPos.from), _clipPos(cm.doc, op.scrollToPos.to), op.scrollToPos.margin);
      if (op.scrollToPos.isCursor && cm.state.focused) maybeScrollWindow(cm, coords);
    }

    if (op.selectionChanged) restartBlink(cm);
    if (cm.state.focused && op.updateInput) resetInput(cm, op.typing); // Fire events for markers that are hidden/unidden by editing or
    // undoing
  ...
  function readInput(cm) {
    var input = cm.display.input,
        ...
        doc = cm.doc; // Since this is called a *lot*, try to bail out as cheaply as
    ...
    cm.display.shift = false; // Find the part of the input that is actually new
    ...
    var inserted = text.slice(same),
        textLines = splitLines(inserted); // When pasing N lines into N selections, insert one line per selection
    ...
    for (var i = doc.sel.ranges.length - 1; i >= 0; i--) {
      var range = doc.sel.ranges[i];
      var from = range.from(),
          to = range.to(); // Handle deletion

      if (same < prevInput.length) from = Pos(from.line, from.ch - (prevInput.length - same)); // Handle overwrite
      else if (cm.state.overwrite && range.empty() && !cm.state.pasteIncoming) to = Pos(to.line, Math.min(getLine(doc, to.line).text.length, to.ch + lst(textLines).length));
      ...
      signalLater(cm, "inputRead", cm, changeEvent); // When an 'electric' character is inserted, immediately trigger a reindent
    ...
    cm.curOp.typing = true; // Don't leave long text in the textarea, since it makes further polling slow
  ...
  function registerEventHandlers(cm) {
    ...
    on(d.scrollbarV, "mousedown", reFocus); // Prevent wrapper from ever scrolling
    ...
    on(window, "resize", onResize); // The above handler holds on to the editor and its data
    // structures. Here we poll to unregister it when the editor is no
    // longer in the document, so that it can be garbage-collected.

    function unregister() {
      if (contains(document.body, d.wrapper)) setTimeout(unregister, 5000);else off(window, "resize", onResize);
    }
    ...
    on(d.input, "copy", prepareCopy); // Needed to handle Tab key in KHTML
  ...
  function leftButtonDown(cm, e, start) {
    ...
    var sel = cm.doc.sel,
        addNew = false; // mac ? e.metaKey : e.ctrlKey;
  ...
  function leftButtonStartDrag(cm, e, start) {
    ...
    var dragEnd = operation(cm, function (e2) {
      ...
      if (Math.abs(e.clientX - e2.clientX) + Math.abs(e.clientY - e2.clientY) < 10) {
        ...
        focusInput(cm); // Work around unexplainable focus problem in IE9 (#2127)
  ...
  function leftButtonSelect(cm, e, start, type, addNew) {
    ...
    if (e.altKey) {
      ...
      start = _posFromMouse(cm, e, true, true);
      ourIndex = -1;
    } else if (type == "double") {
    ...
    function extendTo(pos) {
      ...
      if (type == "rect") {
        ...
        if (type != "single") {
          if (type == "double") var range = findWordAt(doc, pos);else var range = new Range(Pos(pos.line, 0), _clipPos(doc, Pos(pos.line + 1, 0)));
    ...
    function extend(e) {
      ...
      var cur = _posFromMouse(cm, e, true, type == "rect");
  ...
  function onDrop(e) {
    ...
    var pos = _posFromMouse(cm, e, true),
        files = e.dataTransfer.files;

    if (!pos || isReadOnly(cm)) return; // Might be a file drop, in which case we simply extract the text
    // and insert it.
  ...
  function onDragStart(cm, e) {
    ...
    if (signalDOMEvent(cm, e) || eventInWidget(cm.display, e)) return;
    e.dataTransfer.setData("Text", cm.getSelection()); // Use dummy image instead of default browsers image.
  ...
  var maybeTransition; // Handle a key from the keydown event.
  ...
  function onContextMenu(cm, e) {
    ...
    focusInput(cm);
    resetInput(cm); // Adds "Select all" to context menu in FF
  ...
  function indentLine(cm, n, how, aggressive) {
    ...
    if (indentString != curSpaceString) {
      _replaceRange(cm.doc, indentString, Pos(n, 0), Pos(n, curSpaceString.length), "+input");
    } else {
  ...
  CodeMirror.prototype = {
    ...
    posFromMouse: function posFromMouse(e) {
      return _posFromMouse(this, e, true);
    },
    ...
    cursorCoords: function cursorCoords(start, mode) {
      ...
      if (start == null) pos = range.head;else if (_typeof(start) == "object") pos = _clipPos(this.doc, start);else pos = start ? range.from() : range.to();
    ...
    heightAtLine: function heightAtLine(line, mode) {
      ...
      return intoCoordSystem(this, lineObj, {
        top: 0,
        left: 0
      }, mode || "page").top + (end ? this.doc.height - _heightAtLine(lineObj) : 0);
    },
  ...
  eventMixin(CodeMirror); // OPTION DEFAULTS
  ...
  var modes = CodeMirror.modes = {},
      mimeModes = CodeMirror.mimeModes = {}; // Extra arguments are stored as the mode's dependencies, which is
  ...
  CodeMirror.defineMIME("text/plain", "null"); // This can be used to attach properties to mode objects from
  ...
  function _markText(doc, from, to, options, type) {
    ...
    var marker = new TextMarker(doc, type),
        diff = cmp(from, to);
    if (options) copyObj(options, marker); // Don't connect empty markers unless clearWhenEmpty is false
    ...
    if (cm) {
      ...
      if (marker.collapsed) regChange(cm, from.line, to.line + 1);else if (marker.className || marker.title || marker.startStyle || marker.endStyle) for (var i = from.line; i <= to.line; i++) {
  ...
  function widgetHeight(widget) {
    ...
    if (!contains(document.body, widget.node)) removeChildrenAndAdd(widget.cm.display.measure, elt("div", [widget.node], null, "position: relative"));
  ...
  function addLineWidget(cm, handle, node, options) {
    ...
    changeLine(cm, handle, "widget", function (line) {
      ...
      if (!lineIsHidden(cm.doc, line)) {
        var aboveVisible = _heightAtLine(line) < cm.doc.scrollTop;
  ...
  function highlightLine(cm, line, state, forceToEnd) {
    ...
    runMode(cm, line.text, cm.doc.mode, state, function (end, style) {
      st.push(end, style);
    }, forceToEnd); // Run overlays, adjust style array.
  ...
  function buildLineContent(cm, lineView) {
    ...
    for (var i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
      ...
      builder.map = [];
      insertLineContent(line, builder, getLineStyles(cm, line)); // Ensure at least a single node is present, for measuring.
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    replaceSelections: function replaceSelections(code, collapse, origin) {
      var changes = [],
          sel = this.sel;
    ...
    findMarks: function findMarks(from, to) {
      from = _clipPos(this, from);
      to = _clipPos(this, to);
      var found = [],
          lineNo = from.line;
  ...
  eventMixin(Doc); // Call f for all linked documents.
  ...
  function addSelectionToHistory(doc, sel, opId, options) {
    ...
    if (opId == hist.lastOp || origin && hist.lastSelOrigin == origin && (hist.lastModTime == hist.lastSelTime && hist.lastOrigin == origin || selectionEventCanBeMerged(doc, origin, lst(hist.done), sel))) hist.done[hist.done.length - 1] = sel;else pushSelectionToHistory(sel, hist.done);
  ...
  if ([].indexOf) indexOf = function indexOf(array, elt) {
  ...
  if ([].map) map = function map(array, f) {
    return array.map(f);
  };
    ...
    if (colorCache[colorString]) {
      ...
      if (col) {
        ...
        if (r < 2.361) {
          ...
          } while (r < 2.361); // template strings don't work for IE
  ...
  var range;
  if (document.createRange) range = function range(node, start, end) {
  ...
  if (ie_upto10) activeElt = function activeElt() {
    try {
      return document.activeElement;
    } catch (e) {
