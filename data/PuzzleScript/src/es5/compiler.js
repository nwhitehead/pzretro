...
function isColor(str) {
  str = str.trim();
  if (str in colorPalettes.arnecolors) return true;
...
function generateExtraMembers(state) {
  ...
  state.objectCount = idcount; //calculate blank mask template
  ...
  state.STRIDE_MOV = STRIDE_MOV; //get colorpalette name
  ...
  var glyphOrder = []; //calculate glyph dictionary
        ...
        for (var j = 1; j < dat.length; j++) {
          ...
          if (mask[o.layer] == -1) {
            ...
            if (o.layer === undefined) {
              ...
              var n2 = state.idDict[mask[o.layer]].toUpperCase(); // if (n1 !== n2) {

              logError('Trying to create an aggregate object (something defined in the LEGEND section using AND) with both "' + n1 + '" and "' + n2 + '", which are on the same layer and therefore can\'t coexist.', dat.lineNumber); // }
            }
          }
        }
  ...
  glyphOrder.sort(function (a, b) {
    return a[0] - b[0];
  });
  glyphOrder = glyphOrder.map(function (a) {
    return a[1];
  });
  ...
  state.propertiesDict = propertiesDict; //calculate lookup dictionaries
...
var reg_directions_only = /^(\>|\<|\^|v|up|down|left|right|moving|stationary|no|randomdir|random|horizontal|vertical|orthogonal|perpendicular|parallel|action)$/; //redeclaring here, i don't know why
...
function processRuleString(rule, state, curRules) {
  ...
  */
  ...
  var origLine = rule[2]; // STEP ONE, TOKENIZE
  ...
  */
      ...
      case 1:
        {
          if (token == '[') {
            ...
            } else if (incellrow || rightBracketToRightOf(tokens, i)) {
              //only a warning for legacy support reasons.
            ...
            if (token === 'message') {
              ...
              if (messageStr === "") {
                messageStr = " "; //needs to be nonempty or the system gets confused and thinks it's a whole level message rather than an interstitial.
              }
  ...
  if (lhs_cells.length != rhs_cells.length) {
    if (commands.length > 0 && rhs_cells.length == 0) {//ok
    } else {
...
function rulesToArray(state) {
  ...
  state.loops = loops; //now expand out rules with multiple directions
  ...
  for (var i = 0; i < rules2.length; i++) {
    var rule = rules2[i]; //remove relative directions

    convertRelativeDirsToAbsolute(rule); //optional: replace up/left rules with their down/right equivalents

    rewriteUpLeftRules(rule); //replace aggregates with what they mean
  ...
  var rules3 = []; //expand property rules
      ...
      for (var j = 0; j < cur_rule.lhs.length && !shouldremove; j++) {
        ...
        for (var k = 0; k < cur_rulerow.length && !shouldremove; k++) {
          ...
          for (var prop_n = 0; prop_n < properties.length; ++prop_n) {
            ...
            shouldremove = true;
            modified = true; //just do the base property, let future iterations take care of the others
...
function makeSpawnedObjectsStationary(state, rule, lineNumber) {
  ...
  for (var j = 0; j < rule.rhs.length; j++) {
    ...
    for (var k = 0; k < row_r.length; k++) {
      var cell = row_r[k]; //this is super intricate. uff. 

      var objects_l = getPossibleObjectsFromCell(state, row_l[k]);
      var layers = objects_l.map(function (n) {
        return state.objects[n].layer;
      });
...
function concretizeMovingRule(state, rule, lineNumber) {
        ...
        for (var k = 0; k < cur_rulerow.length; k++) {
          ...
          if (movings.length > 0) {
            ...
            for (var l = 0; l < concreteDirs.length; l++) {
              ...
              var newrule = deepCloneRule(cur_rule); //deep copy replacements
  ...
  for (var i = 0; i < result.length; i++) {
    ...
    var ambiguous_movement_dict = {}; //strict first - matches movement direction to objects
    //for each property replacement in that rule
    ...
    for (var cand_name in cur_rule.aggregateDirReplacement) {
      if (cur_rule.aggregateDirReplacement.hasOwnProperty(cand_name)) {
        ...
        var concreteMovement = replacementInfo[0];
        var occurrenceCount = replacementInfo[1];
        var ambiguousMovement = replacementInfo[2]; //are both the following boolean bits necessary, or just the latter? ah well, no harm it seems.
    ...
    for (var j = 0; j < rule.lhs.length; j++) {
      ...
      for (var k = 0; k < cellrow_l.length; k++) {
        ...
        if (objectsPresent.anyBitsInCommon(objectsMissing)) {
          ...
          if (i > 0 && state.rules[i - 1].lineNumber === ln || i + 1 < state.rules.length && state.rules[i + 1].lineNumber === ln) {//all good
          } else {
        ...
        for (var l = 0; l < cell_r.length; l += 2) {
          ...
          if (object_dir === '...') {
            ...
            if (object_name in state.objectMasks) {
              ...
              if (state.propertiesDict.hasOwnProperty(object_name)) {
                ...
                logWarning("In this rule you're asking me to spawn a random ".concat(object_name.toUpperCase(), " for you, but that's already a concrete single object.  You wanna be using random with properties (things defined in terms of OR in the legend) so there's some things to select between."), rule.lineNumber);
                values = [object_name];
              }
          ...
          if (object_dir == 'no') {
            ...
            if (existingname !== null) {
              if (rule.hasOwnProperty('discard')) {} else {
            ...
            } else {// shouldn't need to do anything here...
...
function cellRowMasks(rule) {
  ...
  for (var i = 0; i < lhs.length; i++) {
    ...
    for (var j = 0; j < cellRow.length; j++) {
      if (cellRow[j] === ellipsisPattern) continue;
...
function collapseRules(groups) {
  for (var gn = 0; gn < groups.length; gn++) {
    ...
    for (var i = 0; i < rules.length; i++) {
      var oldrule = rules[i];
      var newrule = [0, [], oldrule.rhs.length > 0, oldrule.lineNumber
      /*ellipses,group number,rigid,commands,randomrule,[cellrowmasks]*/
      ];
      var ellipses = [];
      ...
      if (!(beforesame || aftersame) || ruleGroup.length === 0) {
        ...
        if (discards.length > 1) {
          ...
          for (var j = 1; j < discards.length; j++) {
            ...
            var thisdiscard = discards[j];
            var p1 = thisdiscard[0];
            var p2 = thisdiscard[1];
            parenthetical += "".concat(p1, "/").concat(p2);
        ...
        logError("".concat(example[0], " and ").concat(example[1], " can never overlap").concat(parenthetical, ", but this rule requires that to happen, so it's being culled."), rule.lineNumber);
      }
...
function arrangeRulesByGroupNumber(state) {
  ...
  state.rules = result; //check that there're no late movements with direction requirements on the lhs
...
function generateMasks(state) {
  ...
  state.aggregateMasks = {}; //set aggregate masks similarly

  for (var _i = 0, _Object$keys = Object.keys(state.aggregatesDict); _i < _Object$keys.length; _i++) {
    var aggregateName = _Object$keys[_i];
...
function twiddleMetaData(state) {
  ...
  var getIntCheckedPositive = function getIntCheckedPositive(s, lineNumber) {
    if (!isFinite(s) || !isInt(s)) {
      logWarning("Wasn't able to make sense of \"".concat(s, "\" as a (whole number) dimension."), lineNumber);
      return NaN;
    }
    ...
    if (isNaN(result)) {
      logWarning("Wasn't able to make sense of \"".concat(s, "\" as an dimension."), lineNumber);
    }

    if (result <= 0) {
      logWarning("The dimension given to me (you gave \"".concat(s, "\") is baad - it should be greater than 0."), lineNumber);
    }

    return result;
  };

  var getCoords = function getCoords(str, lineNumber) {
    ...
    if (coords.length !== 2) {
      ...
      if (!isFinite(coords[0]) || !isFinite(coords[1]) || isNaN(intcoords[0]) || isNaN(intcoords[1])) {
        logWarning("Couldn't understand the dimensions given to me (you gave \"".concat(val, "\") - should be of the form AxB."), lineNumber);
        return null;
      } else {
        if (intcoords[0] <= 0 || intcoords[1] <= 0) {
          logWarning("The dimensions given to me (you gave \"".concat(val, "\") are baad - they should be > 0."), lineNumber);
        }
...
function generateSoundData(state) {
  ...
  var sfx_MovementMasks = state.collisionLayers.map(function (x) {
    return [];
  });
  ...
  for (var i = 0; i < state.sounds.length; i++) {
    ...
    var v0 = sound[0][0].trim();
    var t0 = sound[0][1].trim();
    var v1 = sound[1][0].trim();
    var t1 = sound[1][1].trim();
    ...
    if (t0 === "SOUNDEVENT") {
      ...
      for (var j = 2; j < sound.length - 2; j++) {
        //avoid last sound declaration as well as the linenumber element at the end
...
function loadFile(str) {
  ...
  formatHomePage(state); //delete intermediate representations
