    ...
    for (var nidx in row) {
      ...
      if (font.hasOwnProperty(n) && !textImages.hasOwnProperty(n)) {
        fontstr = font[n].split('\n').map(function (a) {
          return a.trim().split('').map(function (t) {
            return parseInt(t);
          });
        });
        fontstr.shift();
...
function generateGlyphImages() {
  ...
  var _iterator = _createForOfIteratorHelper(state.glyphOrder),
      _step;
  ...
  if (IDE) {
    ...
    glyphPrintButton = textImages['editor_s']; //make diff highlighter thingy
    ...
    spritectx.fillRect(0, cellheight - 1, cellwidth, 1);
    spritectx.fillRect(cellwidth - 1, 0, 1, cellheight);
    glyphPrintButton = textImages['editor_s']; //make highlight thingy
    ...
    spritectx.fillRect(0, miny, cellwidth, ysize); //make highlight thingy
    ...
    spritectx.fillRect(0, 0, 2, cellheight);
    spritectx.fillRect(0, cellheight - 2, cellwidth, 2);
    spritectx.fillRect(cellwidth - 2, 0, 2, cellheight); //make movement glyphs
    ...
    */
    ...
    [[3, 8], [5, 10], [7, 8]], //2 left
    [[2, 3], [0, 5], [2, 7]], //3 right
    [[7, 3], [10, 5], [7, 7]], //4 action
    [[3, 5], [5, 7], [7, 5], [5, 3]], //5 rigid
...
function drawEditorIcons(mini, minj) {
  ...
  var glyphEndIndex = glyphImages.length;
  /*Math.min(
  glyphStartIndex+10,
  screenwidth-2,
  ...
  var tooltip_string = '';
  var tooltip_objects = null; // prepare tooltip: legend for highlighted editor icon

  if (mouseCoordX >= 0 && mouseCoordX < screenwidth && mouseIndex >= 0 && mouseIndex < glyphsToDisplay) {
    var _glyphIndex = glyphStartIndex + mouseIndex;

    var identifier_index = glyphImagesCorrespondance[_glyphIndex];
    ...
    var posMask = level.getCellInto(mouseCoordY + minj + (mouseCoordX + mini) * level.height, _o12);
    tooltip_objects = state.idDict.filter(function (x, k) {
      return posMask.get(k) != 0;
