...
function selectText(containerid, e) {
  ...
  if (e && (e.ctrlKey || e.metaKey)) {
    ...
    if (document.selection) {
      ...
      var selection = window.getSelection(); //why removeallranges? https://stackoverflow.com/a/43443101 whatever...
...
function matchGlyph(inputmask, glyphAndMask) {
  ...
  for (var i = 0; i < glyphAndMask.length; ++i) {
    ...
    var glyphbits = glyphAndMask[i][2]; //require all bits of glyph to be in input

    if (glyphmask.bitsSetInArray(inputmask.data)) {
      var bitcount = 0;

      for (var bit = 0; bit < 32 * STRIDE_OBJ; ++bit) {
        if (glyphbits.get(bit) && inputmask.get(bit)) bitcount++;
...
function printLevel() {
  ...
  for (var glyphName in state.glyphDict) {
    if (state.glyphDict.hasOwnProperty(glyphName) && glyphName.length === 1) {
      ...
      var glyphbits = glyphmask.clone(); //register the same - backgroundmask with the same name
...
function onKeyDown(event) {
  ULBS();
  event = event || window.event; // Prevent arrows/space from scrolling page

  if (!IDE && [32, 37, 38, 39, 40].indexOf(event.keyCode) > -1) {
    if (event && (event.ctrlKey || event.metaKey)) {} else {
      prevent(event);
    }
  }
...
function mouseMove(event) {
  ...
  event.handled = true; //window.console.log("showcoord ("+ canvas.width+","+canvas.height+") ("+x+","+y+")");
}

function mouseOut() {//  window.console.log("clear");
}
...
function checkKey(e, justPressed) {
  ...
  if (textMode) {
    if (state.levels.length === 0) {//do nothing
    } else if (titleScreen) {
    ...
    if (!againing && inputdir >= 0) {
      if (inputdir === 4 && 'noaction' in state.metadata) {} else {
        pushInput(inputdir);
...
var looping = false; // Lights, camera…function!
