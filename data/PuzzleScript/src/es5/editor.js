...
CodeMirror.commands.swapLineUp = function (cm) {
  ...
  cm.operation(function () {
    for (var i = 0; i < linesToMove.length; i += 2) {
      ...
      if (to > cm.lastLine()) cm.replaceRange("\n" + line, CodeMirror.Pos(cm.lastLine()), null, "+swapLine");else cm.replaceRange(line + "\n", CodeMirror.Pos(to, 0), null, "+swapLine");
    }
...
CodeMirror.commands.swapLineDown = function (cm) {
  ...
  for (var i = ranges.length - 1; i >= 0; i--) {
    var range = ranges[i],
        ...
        to = range.from().line; //   if (range.to().ch == 0 && !range.empty()) from--;
  ...
  cm.operation(function () {
    for (var i = linesToMove.length - 2; i >= 0; i -= 2) {
      var from = linesToMove[i],
          to = linesToMove[i + 1];
      var line = cm.getLine(from);
      if (from == cm.lastLine()) cm.replaceRange("", CodeMirror.Pos(from - 1), CodeMirror.Pos(from), "+swapLine");else cm.replaceRange("", CodeMirror.Pos(from, 0), CodeMirror.Pos(from + 1, 0), "+swapLine");
...
function addToDebugTimeline(level, lineNumber) {
  ...
  var debugTimelineSnapshot = {
    ...
    rigidMovementAppliedMask: level.rigidMovementAppliedMask.map(function (a) {
      return a.clone();
    })
  };
  debug_visualisation_array[debugger_turnIndex][lineNumber] = debugTimelineSnapshot;
  return "".concat(debugger_turnIndex, ",").concat(lineNumber);
}