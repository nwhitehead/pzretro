...
function dumpTestCase() {
  ...
  resultstring = "<br>\n\t[<br>\n\t\t\"".concat(state.metadata.title || "untitled test", "\",<br>\n\t\t").concat(resultstring, "<br>\n\t],");
  selectableint++;
  var tag = 'selectable' + selectableint;
  consolePrint("<br>Compilation error/warning data (for error message tests - errormessage_testdata.js):<br><br><br><span id=\"" + tag + "\" onclick=\"selectText('" + tag + "',event)\">" + resultstring + "</span><br><br><br>", true); //if the game is currently running and not on the title screen, dump the recording data
