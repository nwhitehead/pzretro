 ...
 */
window.Mobile = {}; //stolen from https://github.com/Modernizr/Modernizr/blob/master/feature-detects/touchevents.js
...
(function (proto) {
  'use strict'; // Minimum range to begin looking at the swipe direction, in pixels

  var SWIPE_THRESHOLD = 10; // Distance in pixels required to complete a swipe gesture.

  var SWIPE_DISTANCE = 50; // Time in milliseconds to complete the gesture.

  var SWIPE_TIMEOUT = 1000; // Time in milliseconds to repeat a motion if still holding down,
  // ... and not specified in state.metadata.key_repeat_interval.

  var DEFAULT_REPEAT_INTERVAL = 150; // Lookup table mapping action to keyCode.
  ...
  proto.swipeStep = function (event) {
    ...
    if (!this.swipeDirection) {
      ...
      direction = this.dominantDirection(this.firstPos, currentPos); // Cancel the swipe if the direction changes.
  ...
  proto.emitKeydown = function (input) {
    ...
    this.fakeCanvasFocus(); // Press, then release key.
  ...
  proto.buildTab = function () {
    ...
    this.tabElem = assemblyElem.getElementsByClassName('tab-icon')[0]; //the reason I'm adding all these empty click events is on safari to disable double-tap to zoom (also needs some other css settings. cf https://github.com/increpare/PuzzleScript/issues/599 SMGDH)

    this.tabAffordance.addEventListener('touchstart', openCallback);
    this.tabAffordance.addEventListener("click", function (event) {});
  ...
  proto.buildMenu = function () {
    ...
    this.closeAffordance.addEventListener("click", function (event) {});
  ...
  proto.setTabAnimationRatio = function (ratio) {
    ...
    var style; // Round away any exponents that might appear.
